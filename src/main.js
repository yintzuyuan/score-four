...
function tryDrop(x, z) {
  ...
  if (winLine) {
    ...
    setTimeout(() => showWinner(winningPlayer), 1100);
...
function animate() {
  ...
  if (winLineMesh) {
    const t = performance.now() * 0.003;
    winLineMesh.material.opacity = 0.7 + Math.sin(t * 1.5) * 0.2;
  }
